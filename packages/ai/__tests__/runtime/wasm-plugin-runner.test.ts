 ...
 *
 * Kept intentionally minimal to avoid runtime-specific WASI binary
 * compatibility issues across Node versions.
 */
const EXIT_CODE_42_WASM_BYTES = new Uint8Array([
  0x00, 0x61, 0x73, 0x6d, // Magic number: \0asm
  0x01, 0x00, 0x00, 0x00, // Version: 1
  // Type section: function signature (void -> i32)
  0x01, 0x05, 0x01, 0x60, 0x00, 0x01, 0x7f,
  // Function section: 1 function of type 0
  0x03, 0x02, 0x01, 0x00,
  // Export section: export function 0 as "_start"
  0x07, 0x0a, 0x01, 0x06, 0x5f, 0x73, 0x74, 0x61, 0x72, 0x74, 0x00, 0x00,
  // Code section: function body returns i32.const 42
  0x0a, 0x06, 0x01, 0x04, 0x00, 0x41, 0x2a, 0x0b,
]);

/**
 * WASM module that returns code 3.
 * Used by memory-limit tests to assert successful execution path.
 */
const MEMORY_LIMIT_WASM_BYTES = new Uint8Array([
  ...
  // Export section: export function 0 as "_start"
  0x07, 0x0a, 0x01, 0x06, 0x5f, 0x73, 0x74, 0x61, 0x72, 0x74, 0x00, 0x00,
  // Code section: function body returns i32.const 3
  0x0a, 0x06, 0x01, 0x04, 0x00, 0x41, 0x03, 0x0b,
]);
...
describe('WasmPluginRunner.run() — basic execution', () => {
  ...
  it('should capture non-zero exit code from _start return value', async () => {
    const result = await WasmPluginRunner.run({
