...
export interface DelegationTelemetryEvent {
  ...
  /** Workspace identifier (repo path or slug) */
  workspace_id?: string;

  /** Trace correlation ID */
  trace_id?: string;

  /** Node identifier where event was emitted */
  node_id?: string;

  /** Repository identifier in owner/repo format when available */
  repo?: string;
...
export class DelegationTelemetryEngine extends EventEmitter {
  ...
  private async emitEvent(eventData: Partial<DelegationTelemetryEvent>): Promise<void> {
    ...
    const workspaceId =
      (eventDataRecord.workspace_id as string | undefined) ||
      process.env.DCYFR_TELEMETRY_WORKSPACE_ID ||
      process.cwd();
    const nodeId =
      (eventDataRecord.node_id as string | undefined) ||
      process.env.DCYFR_TELEMETRY_NODE_ID ||
      process.env.HOSTNAME ||
      'unknown-node';
    const repo =
      (eventDataRecord.repo as string | undefined) ||
      process.env.DCYFR_TELEMETRY_REPO ||
      process.env.GITHUB_REPOSITORY ||
      basename(workspaceId);
    const traceId =
      (eventDataRecord.trace_id as string | undefined) ||
      randomUUID();
    const schemaVersion =
      (eventDataRecord.schema_version as string | undefined) ||
      process.env.DCYFR_TELEMETRY_SCHEMA_VERSION ||
      '1.0.0';
